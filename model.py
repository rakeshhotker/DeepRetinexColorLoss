...
def RelightNet(input_L, input_R, channel=64, kernel_size=3):
    ...
    with tf.variable_scope('RelightNet'):
        ...
        up1 = tf.image.resize(conv3, (tf.shape(conv2)[1], tf.shape(conv2)[2]), method=tf.image.ResizeMethod.BICUBIC)
        deconv1 = tf.layers.conv2d(up1, channel, kernel_size, padding='same', activation=tf.nn.relu) + conv2
        up2 = tf.image.resize(deconv1, (tf.shape(conv1)[1], tf.shape(conv1)[2]), method=tf.image.ResizeMethod.BICUBIC)
        deconv2= tf.layers.conv2d(up2, channel, kernel_size, padding='same', activation=tf.nn.relu) + conv1
        up3 = tf.image.resize(deconv2, (tf.shape(conv0)[1], tf.shape(conv0)[2]), method=tf.image.ResizeMethod.BICUBIC)
        deconv3 = tf.layers.conv2d(up3, channel, kernel_size, padding='same', activation=tf.nn.relu) + conv0
        
        deconv1_resize = tf.image.resize(deconv1, (tf.shape(deconv3)[1], tf.shape(deconv3)[2]), method=tf.image.ResizeMethod.BICUBIC)
        deconv2_resize = tf.image.resize(deconv2, (tf.shape(deconv3)[1], tf.shape(deconv3)[2]), method=tf.image.ResizeMethod.BICUBIC)
...
class lowlight_enhance(object):
    def __init__(self, sess):
        ...
        self.color_loss=self.color_difference(R_low*I_delta_3,self.input_high)#color_loss
#         self.ssim_loss_value=self.ssim_loss(R_low*I_delta_3,self.input_high)#ssim_loss
        ...
        self.loss_Relight = self.relight_loss + 3 * self.Ismooth_loss_delta+ 0.1* self.color_loss
    ...
    def color_difference(self,img1, img2):
        # Convert images to float32 data type
        img1 = tf.cast(img1, tf.float32)
        img2 = tf.cast(img2, tf.float32)

        # Calculate Euclidean distance between RGB pixel values of both images
        dist = tf.sqrt(tf.reduce_sum(tf.square(img1 - img2), axis=-1))

        # Return the mean color difference
        return tf.reduce_mean(dist)
    
    def ssim_loss(self, X, Y):
        ssim_loss = tf.image.ssim(X, Y, max_val=1.0)
    ...
    def test(self, test_low_data, test_high_data, test_low_data_names, save_dir, decom_flag):
        ...
        load_model_status_Decom, _ = self.load(self.saver_Decom, './NewCheckpoint/Decom')
        load_model_status_Relight, _ = self.load(self.saver_Relight, './NewCheckpoint/Relight')
